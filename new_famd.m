...
%         A(ad_in)=10+5*randn;
    ...
    T=250
    T_Beta=500*rand(order*noOfNodes,noOfNodes,T);
    ...
    for ii=T+order+1:noOfObservations
        ...
        for k=1:noOfNodes
            ...
            for m=1:order
                for l=1:noOfNodes
                    temp=temp+ A(noOfNodes *(m-1)+k,l)*((1*gscale*exp(-1000000*(M_x(l,ii-m-T:ii-m-1)-M_x(l,ii-m)).^2/(10*gsigma*gsigma))*reshape(T_Beta(noOfNodes *(m-1)+k,l,:),[T,1]))+(0*gscale*exp(-1000000*(M_x(l,ii-m-T:ii-m-1)-M_x(l,ii-m)).^2/(2*gsigma*gsigma))*reshape(T_Beta(noOfNodes *(m-1)+k,l,:),[T,1])));
    ...
    [pred_RF1,alpha_RF1,mse_RF1(kk,:),pmdRF1(kk,:),pfaRF1(kk,:)]=NLTISORFF(order,lamda2,gamma_RF,vsigma,m_X,noOfNodes,filtOrder,noOfObservations,10,A1,A2,A3);
    [pred_RF2,alpha_RF2,mse_RF2(kk,:),pmdRF2(kk,:),pfaRF2(kk,:)]=NLTISORFF(order,lamda2,gamma_RF,vsigma,m_X,noOfNodes,filtOrder,noOfObservations,30,A1,A2,A3);
    [pred_RF3,alpha_RF3,mse_RF3(kk,:),pmdRF3(kk,:),pfaRF3(kk,:)]=NLTISORFF(order,lamda2,gamma_RF,vsigma,m_X,noOfNodes,filtOrder,noOfObservations,50,A1,A2,A3);
        [pred_NLTISO,alpha_NLTISO,mse_NL(kk,:),pmd1(kk,:),pfa1(kk,:)]=NLTISOF(order,lamda1,gamma_NLTISO,Ksigma,m_X,noOfNodes,filtOrder,noOfObservations,D,A1,A2,A3);
...
figure
plot((sum(pmd1))/100,'LineWidth',2);
hold on
plot((sum(pmdRF1))/100,'LineWidth',2);
hold on
plot((sum(pmdRF2))/100,'LineWidth',2);
hold on
plot((sum(pmdRF3))/100,'LineWidth',2);
% title('Probability of miss detection ')
lgd2=legend('NL-TISO','RF-NLTISO:D=10','RF-NLTISO:D=30','RF-NLTISO:D=50')
set(lgd2,'fontsize',24)
ylabel('P_{MD}','FontSize',24)
xlabel('t','FontSize',24)

figure
plot(sum(pfa1)/100,'LineWidth',2);
hold on
plot(sum(pfaRF1)/100,'LineWidth',2);
hold on
plot(sum(pfaRF2)/100,'LineWidth',2);
hold on
plot(sum(pfaRF3)/100,'LineWidth',2);
% title('Probability of false alarm detection ')
lgd3=legend('NL-TISO','RF-NLTISO:D=10','RF-NLTISO:D=30','RF-NLTISO:D=50')
set(lgd3,'fontsize',24)
ylabel('P_{FA}','FontSize',24)
xlabel('t','FontSize',24)
