...
clear all
%% Data generation
noOfNodes        = 10; %no of nodes
filtOrder        = 4; % process order
...
RFObj_tirso.vsigma    =1*ones(RFObj_tirso.noOfNodes,1);
...
c.LineWidth = 1.5;
set(c,'fontSize',24);
%% RFNL-TISO ESTIMATION AND PLOTTING    
addpath '/Users/rohanmoney/git_rohan/NonLinearTISO'
...
%   plot((RF_m_prediction(sensor_to_plot,1:end-12)-m_X(sensor_to_plot,13:end)).^2)
 error_temp=(RF_m_prediction_tirso(sensor_to_plot,1:end-1)-m_X(sensor_to_plot,2:end)).^2;
 for ii=1:length(error_temp)
     nmse(ii)=sum(error_temp(1:ii))/sum((m_X(sensor_to_plot,2:ii+1)).^2);
 end
 hold on
 plot(nmse)
 hold on
  error_temp=(RF_m_prediction_tiso(sensor_to_plot,1:end-1)-m_X(sensor_to_plot,2:end)).^2;
 for ii=1:length(error_temp)
     nmse(ii)=sum(error_temp(1:ii))/sum((m_X(sensor_to_plot,2:ii+1)).^2);
 end
 hold on
 plot(nmse)
  error_temp=(m_prediction(sensor_to_plot,1:end-1)-m_X(sensor_to_plot,2:end)).^2;
 for ii=1:length(error_temp)
     nmse(ii)=sum(error_temp(1:ii))/sum((m_X(sensor_to_plot,2:ii+1)).^2);
 end
 hold on
 plot(nmse)
 legend('RFNL-TIRSO','RFNL-TISO','TIRSO')