...
%         A(ad_in)=10+5*randn;
    ...
    [pred_RF1,alpha_RF1,mse_RF1(kk,:),pmdRF1(kk,:),pfaRF1(kk,:)]=NLTISORFF(order,lamda,gamma_RF,vsigma,m_X,noOfNodes,filtOrder,noOfObservations,10,A1,A2,A3);
...
lgd2=legend('NL-TISO','RF-NLTISO-D=10','RF-NLTISO-D=30','RF-NLTISO-D=100')
...
lgd3=legend('NL-TISO','RF-NLTISO-D=10','RF-NLTISO-D=30','RF-NLTISO-D=100')
...
for t=filtOrder+1:noOfObservations
    if t<1000
        ...
        MF=wthresh(C-A1,'h',.5);
         MF(isnan(MF))=0;
        pfa(t)=(sum(sum(MF))-1)/50;
           indmd=find(A1);
        
        cv=wthresh(1-abs(A1(indmd)-C(indmd)),'h',.4);
        cv(isnan(cv))=1;
        cunt=nnz(cv);
        tcount=nnz(indmd);
    ...
    elseif t<2000
        ...
        cv=wthresh(1-abs(A2(indmd)-C(indmd)),'h',.4);
         cv(isnan(cv))=1;
        cunt=nnz(cv);
        tcount=nnz(indmd);
...
for t=filtOrder+1:noOfObservations
    ...
    elseif t<2000
       ...
       pmd(t)=(tcount-cunt)/ tcount;
        ...
        countmd=0;
        indmd=find(A3);
        MF=wthresh(C-A1,'h',.49);
        MF(isnan(MF))=0;
        pfa(t)=(sum(sum(MF))-1)/50;
        
         indmd=find(A3);
        
        cv=wthresh(1-abs(A3(indmd)-C(indmd)),'h',.4);
         cv(isnan(cv))=1;
        cunt=nnz(cv);
        tcount=nnz(indmd);
