...
M_x=.5*randn(noOfNodes ,noOfObservations );
...
for ii=order+1+T:noOfObservations
    ...
    for k=1:noOfNodes
        ...
        for m=1:order
            ...
            for l=1:noOfNodes
                temp=temp+ ((exp(-1000*(flip(M_x(l,ii-m-1-T+1:ii-m-1))-M_x(l,ii-m)).^2/(10*gsigma*gsigma))))*vec(adjacency_real(k,l,m,:));
                adjacency_prev=adjacency_real(k,l,m,:);
                if norm(vec(adjacency_real(k,l,m,:)))>0
                    
                    adjacency_real(k,l,m,:)=adjacency_real(k,l,m,:)+.1*sin(ii);
                end
