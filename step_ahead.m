...
% [pred_RF1,alpha_RF1,mse1,ti1,npred1]=NLTISORFF(order,lamda,gamma_RF,vsigma,m_X,noOfNodes,filtOrder,noOfObservations,10,step);
 [pred_RF2,alpha_RF2,mse2,ti2,npred2]=NLTISORFF(order,lamda,gamma_RF,vsigma,m_X,noOfNodes,filtOrder,noOfObservations,100,step);
...
for t = tirsoObj.order+1:nTimeInstants
    ...
    m_predic(:,:)=tState_in.predictManyFromBuffer(30)';
    m_prediction(1:14,t)= m_predic(:,10);
      if t<3991
...
plot(m_X(4,10:noOfObservations),'LineWidth',2)
hold on
...
% % plot(movavg_pred(7,:))
% % hold on
 plot(m_prediction(4,:),'LineWidth',2)
