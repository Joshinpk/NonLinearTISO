...
%         A(ad_in)=10+5*randn;
    ...
    T=20
    T_Beta=ones(order*noOfNodes,noOfNodes,noOfObservations);
    ...
    for ii=order+1+T:noOfObservations
        ...
        for k=1:noOfNodes
            ...
            for m=1:order
                for l=1:noOfNodes
                    temp=temp+ A(noOfNodes *(m-1)+k,l)*exp(-1000*(M_x(l,1:ii-m-1)-M_x(l,ii-m)).^2/(10*gsigma*gsigma))*(reshape(T_Beta(noOfNodes *(m-1)+k,l,1:ii-m-1),[ii-m-1,1]).*kernal_forget(1:ii-m-1)');
...
RFObj_tirso.vsigma    =1;
  RFObj_tirso.forgettingFactor=.001;
